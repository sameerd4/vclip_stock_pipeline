...
class ContentReadinessService:
    ...
    def __init__(self, catalog: WorkflowCatalog) -> None:
        ...
        self.review = ReviewService(catalog)
    ...
    def prepare(
        ...
        *,
        ...
        resolved_version = resolve_collection_version(self.catalog, slug=slug, version=version)
        ...
        return {
            ...
            "rights_review_path": review.get("rights_review_path"),
            "rights_evidence_path": review.get("rights_evidence_path"),
            "clip_count": public.get("clip_count"),
            "status": "prepared",
            "note": (
                "Public metadata and rights-review evidence/template are ready. "
                "Content readiness still requires human confirmation and review validate."
            ),
        }
    ...
    def validate(
        ...
        *,
        ...
        if not public_path.is_file():
            public_failures.append(f"public-metadata.json is missing at {public_path}")
            public = None
        else:
        ...
        review_result = self.review.validate(
            slug=slug,
            version=resolved_version,
            release_root=release_root,
        )
        review_failures = list(review_result["failures"])
        review_ready = bool(review_result["review_ready"])

        # Review validation may add customer-safe rights notices.
        if public_path.is_file():
            public = load_json(public_path)
            public_failures = validate_public_metadata(public, package)
            public_ready = not public_failures
        failures = [*public_failures, *review_failures]

        status = "content_ready" if not failures else "not_content_ready"
        result = {
            "status": status,
            "public_metadata_ready": public_ready,
            "rights_review_ready": review_ready,
            "review_validation_ready": review_ready,
